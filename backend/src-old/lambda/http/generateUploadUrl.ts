...
import { OK_STATUS_CODE, SERVER_ERROR_STATUS_CODE } from '../../utils/constants'
...
export const handler = middy(
  async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
      ...
      const uploadUrl = generateSignedUrl(userId)

      logger.info('Generating signed url ', uploadUrl)
    ...
    if (uploadAttachmentUrlRes instanceof CustomError) {
      ...
      return {
        ...
        body: JSON.stringify({ uploadUrl, attachmentId })
      }
    } catch (error) {
      return {
